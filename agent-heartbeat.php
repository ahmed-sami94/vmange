...
if (!rate_limit('agent-heartbeat', 240, 60)) {
...
function heartbeat_b64(string $key): string
{
    $decoded = base64_decode((string) ($_POST[$key] ?? ''), true);
    return $decoded === false ? '' : $decoded;
}

$allVms = heartbeat_b64('all_vms');
$runningVms = heartbeat_b64('running_vms');
$vmSpecs = heartbeat_b64('vm_specs');
$vmInventoryJson = heartbeat_b64('vm_inventory_json');
$metricsJson = heartbeat_b64('metrics_json');
...
$containersJson = heartbeat_b64('containers_json');
$composeJson = heartbeat_b64('compose_json');
$imagesJson = heartbeat_b64('images_json');
$capabilitiesJson = heartbeat_b64('capabilities_json');
$collectorErrorsJson = heartbeat_b64('collector_errors_json');
...
if ($commandId > 0) {
    ...
    $commandOutput = substr(heartbeat_b64('command_output'), 0, 4000);
