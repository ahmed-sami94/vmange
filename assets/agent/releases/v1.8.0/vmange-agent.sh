...
collect_capabilities() {
  local has_virtualbox=false has_docker=false has_compose=false has_root_helper=false docker_bin="" compose_bin="" vbox_bin=""
  if [ -n "${VBOXMANAGE_BIN:-}" ]; then
  ...
  printf '{"has_virtualbox":%s,"has_docker":%s,"has_compose":%s,"has_root_helper":%s,"vboxmanage_bin":"%s","docker_bin":"%s","compose_bin":"%s","run_user":"%s","home":"%s","uptime_seconds":%d}\n' \
    "$has_virtualbox" "$has_docker" "$has_compose" "$has_root_helper" \
...
run_root_helper() {
  ...
  if [ "$(id -u)" -eq 0 ]; then
    ...
    sudo -n /usr/local/sbin/vmange-root-helper "$@"
  else
    return 126
  fi
}
...
PY
    ...
    terminal_exec)
      ...
      ;;
    agent_upgrade)
      local requested_url expected_checksum expected_version trusted_prefix new_version
