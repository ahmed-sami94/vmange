...
function b64_field(string $key): string
{
    $decoded = base64_decode((string) ($_POST[$key] ?? ''), true);
    return $decoded === false ? '' : $decoded;
}

$allVms = b64_field('all_vms');
$runningVms = b64_field('running_vms');
$vmSpecs = b64_field('vm_specs');
$vmInventoryJson = b64_field('vm_inventory_json');
$metricsJson = b64_field('metrics_json');
...
$containersJson = b64_field('containers_json');
$composeJson = b64_field('compose_json');
$imagesJson = b64_field('images_json');
$capabilitiesJson = b64_field('capabilities_json');
$collectorErrorsJson = b64_field('collector_errors_json');
...
if ($commandId > 0) {
    ...
    $commandOutput = substr(b64_field('command_output'), 0, 4000);
